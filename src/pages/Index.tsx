...
export default function Index() {
  const [activeTab, setActiveTab] = useState<"latam" | "paises" | "categoria">(
    "latam"
  );

  // Estados para filtro de fechas (aplicable a todas las tabs)
  ...
  const [selectedCategoriaView, setSelectedCategoriaView] = useState<
    string | null
  >(null);
  const [selectedRelacionPepsico, setSelectedRelacionPepsico] = useState<
    string[]
  >([]);
  ...
  const filteredPromotions = useMemo(() => {
    ...
    // Filtro por fecha de análisis (aplicable a todas las tabs)
    if (startDate || endDate) {
      promos = promos.filter((p) => {
        const fechaAnalisis = new Date(p.fechaAnalisis);
        if (startDate && endDate) {
          return fechaAnalisis >= startDate && fechaAnalisis <= endDate;
        } else if (startDate) {
          return fechaAnalisis >= startDate;
        } else if (endDate) {
          return fechaAnalisis <= endDate;
        }
        return true;
      });
    }
    ...
    if (activeTab === "latam") {
      ...
      if (selectedCelebrities.length > 0) {
        promos = promos.filter(
          (p) =>
            p.concepto === "CELEBRITIES" &&
            selectedCelebrities.includes(p.celebrity || "")
        );
      }
      ...
      if (countryFilters.categorias.length > 0) {
        promos = promos.filter((p) =>
          countryFilters.categorias.includes(p.categoria)
        );
      }
      if (countryFilters.tipoPromo.length > 0) {
        promos = promos.filter((p) =>
          countryFilters.tipoPromo.includes(p.tipoPromo)
        );
      }
      ...
      if (selectedRelacionPepsico.length > 0) {
        promos = promos.filter((p) =>
          selectedRelacionPepsico.includes(p.relacionPepsico)
        );
      }
    }
  ...
  const downloadCSV = () => {
    ...
    link.download = `promociones_${activeTab}_${
  ...
  const celebrities = useMemo(() => {
    return Array.from(
      new Set(
        mockPromotions.filter((p) => p.celebrity).map((p) => p.celebrity!)
      )
    );
  }, []);
      ...
      <div className="max-w-[1800px] mx-auto space-y-6">
        ...
        >
          ...
          <TabsContent value="latam" className="space-y-6">
            ...
            <Card className="p-6">
              ...
              <div className="grid grid-cols-5 gap-4">
                {(
                  ...
                  const count = filteredPromotions.filter(
                    (p) => p.pais === country
                  ).length;
                  return (
                    <div
              ...
              <div className="space-y-4">
                ...
                {selectedCategorias.length > 0 && (
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selectedCategorias
                        .flatMap((cat) => mexicoSubcategorias[cat] || [])
                        .map((subcat) => (
                          <Button
                            key={subcat}
                            variant={
                              selectedSubcategorias.includes(subcat)
                                ? "default"
                                : "outline"
                            }
                            size="sm"
            ...
            <Card className="p-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-2xl font-bold">Promociones por Deporte</h2>
                <div className="text-2xl font-bold text-blue-600">
                  {deportes.reduce((sum, deporte) => {
              ...
              <div className="flex flex-wrap gap-2">
                {celebrities.map((celebrity) => (
                  <Button
                    key={celebrity}
                    variant={
                      selectedCelebrities.includes(celebrity)
                        ? "default"
                        : "outline"
                    }
                    size="sm"
              ...
              <div className="space-y-4">
                <div className="bg-white p-4 rounded-lg">
                  ...
                  <ul className="list-disc list-inside space-y-1 text-slate-700">
                    <li>
                      Las bebidas gaseosas lideran con el 28% de las promociones
                      totales
                    </li>
                    <li>
                      Promociones transaccionales representan el 65% del total
                    </li>
                    <li>
                      Brasil y México concentran el 55% de las activaciones
                    </li>
                  </ul>
                </div>
                <div className="bg-white p-4 rounded-lg">
                  ...
                  <ul className="list-disc list-inside space-y-1 text-slate-700">
                    <li>
                      Incrementar presencia en categoría de snacks salados
                      (oportunidad de crecimiento)
                    </li>
                    <li>
                      Fortalecer alianzas deportivas, especialmente en Mundial y
                      UEFA
                    </li>
                    <li>
                      Considerar más promociones de engagement para mejorar
                      fidelización
                    </li>
                  </ul>
                </div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {mexicoCategorias.map((cat) => (
                          <Button
                            key={cat}
                            variant={
                              countryFilters.categorias.includes(cat)
                                ? "default"
                                : "outline"
                            }
                            size="sm"
          ...
          <TabsContent value="categoria" className="space-y-6">
            <Card className="p-6">
              <h2 className="text-2xl font-bold mb-4">
                Análisis por Categoría y Competencia
              </h2>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {mexicoCategorias.map((cat) => (
                      <Button
                        key={cat}
                        variant={
                          selectedCategoriaView === cat ? "default" : "outline"
                        }
                        size="sm"
                  ...
                  <div className="flex gap-2">
                    {[
                      ...
                      <Button
                        key={rel}
                        variant={
                          selectedRelacionPepsico.includes(rel)
                            ? "default"
                            : "outline"
                        }
                        onClick={() => {
                ...
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <Card className="p-6 bg-orange-50">
                    ...
                    <div className="text-3xl font-bold text-orange-600">
                      {
                        filteredPromotions.filter(
                          (p) => p.relacionPepsico === "COMPETENCIA DIRECTA"
                        ).length
                      }
                    </div>
                    <p className="text-sm text-slate-600">
                  ...
                  <Card className="p-6 bg-purple-50">
                    ...
                    <div className="text-3xl font-bold text-purple-600">
                      {
                        filteredPromotions.filter(
                          (p) => p.relacionPepsico === "COMPETENCIA OTROS"
                        ).length
                      }
                    </div>
